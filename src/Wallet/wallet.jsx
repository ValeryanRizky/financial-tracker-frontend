...
export default function WalletPage() {
    ...
    // 🔥 WARNA ICON BERDASARKAN TIPE WALLET
    ...
    const getWalletIcon = (wallet) => {
        ...
        const iconClass = `w-10 h-10 rounded-xl flex items-center justify-center ${iconColor}`;

        switch (wallet.type) {
            case 'bank': return <div className={iconClass}><Building2 size={20} /></div>;
            case 'ewallet': return <div className={iconClass}><Smartphone size={20} /></div>;
            case 'cash': return <div className={iconClass}><Banknote size={20} /></div>;
            case 'credit': return <div className={iconClass}><CreditCard size={20} /></div>;
            case 'debit': return <div className={iconClass}><CreditCard size={20} /></div>;
            case 'qris': return <div className={iconClass}><QrCode size={20} /></div>;
            case 'transfer': return <div className={iconClass}><TrendingUp size={20} /></div>;
            default: return <div className={iconClass}><Wallet size={20} /></div>;
        }
    };

    const getCategoryIcon = (category) => {
        switch (category) {
            case 'payment': return <Building2 size={16} className="text-blue-500" />;
            case 'digital': return <Smartphone size={16} className="text-purple-500" />;
            case 'credit': return <CreditCard size={16} className="text-amber-500" />;
            case 'savings': return <Award size={16} className="text-emerald-500" />;
                ...
                <div className="flex-1 flex items-center justify-center">
                    <div className="text-center">
                        <motion.div
                            ...
                            className="w-12 h-12 border-3 border-indigo-200 border-t-indigo-500 rounded-full mx-auto mb-4"
                        />
                        <p className="text-slate-500 text-sm">Loading your portfolio...</p>
                    </div>
                </div>
            </div>
        );
            ...
            <div className="flex-1 overflow-y-auto">
                <div className="max-w-7xl mx-auto px-6 py-8">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                        <div>
                            <div className="flex items-center gap-2">
                                <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center shadow-lg shadow-indigo-200">
                                    <Gem size={16} className="text-white" />
                                </div>
                                <span className="font-black text-gray-800 drop-shadow-sm text-lg">
                                    Wallet
                                </span>
                            </div>
                        </div>
                        <div className="flex gap-3">
                            ...
                            <motion.button
                                ...
                                className="px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl font-bold shadow-lg shadow-indigo-200 hover:shadow-xl transition-all flex items-center gap-2"
                            >
                                <Plus size={18} />
                    ...
                    <motion.div
                        ...
                        className="relative bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl p-6 md:p-8 mb-8 overflow-hidden"
                    >
                        ...
                        <div className="relative">
                            <div className="flex items-center justify-between mb-4">
                                ...
                                >
                                    {showBalance ? <Eye size={14} className="text-white/70" /> : <EyeOff size={14} className="text-white/70" />}
                                </button>
                            </div>
                            ...
                            <div className="flex flex-wrap items-center gap-6 pt-4 border-t border-white/20">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-xs text-indigo-100">Total Assets</span>
                                    <span className="text-sm font-bold text-white">{formatCompactCurrency(displayTotal)}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-2 h-2 rounded-full bg-purple-300" />
                                    <span className="text-xs text-indigo-100">Active Accounts</span>
                                    <span className="text-sm font-bold text-white">{wallets.length}</span>
                                </div>
                            </div>
                        </div>

                        <Sparkles className="absolute bottom-4 right-4 w-8 h-8 text-white/20" />
                    </motion.div>
                                                ...
                                                >
                                                    ...
                                                    <div className="relative bg-white rounded-xl overflow-hidden">
                                                        <div className="p-5">
                                                            ...
                                                            <h4 className="text-lg font-bold text-slate-800 mb-1">{wallet.name}</h4>
                                                            {wallet.institution && (
                                                                <p className="text-xs text-slate-400">{wallet.institution}</p>
                                                            )}
                    ...
                    {wallets.length === 0 && (
                        ...
                        >
                            ...
                            <p className="text-slate-500 font-medium mb-2">No accounts yet</p>
                            <p className="text-sm text-slate-400 mb-4">Start building your financial portfolio</p>
                            <button
                                onClick={() => setIsModalOpen(true)}
                                className="px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl font-medium shadow-md hover:shadow-lg transition-all"
                            >
